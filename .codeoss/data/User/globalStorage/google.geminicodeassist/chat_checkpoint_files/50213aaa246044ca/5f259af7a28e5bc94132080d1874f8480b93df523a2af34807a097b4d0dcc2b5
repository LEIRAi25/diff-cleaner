...
env = os.environ
# The PROJECT_ID should be provided by the Cloud Run environment as GOOGLE_CLOUD_PROJECT.
# As a fallback for local testing or misconfigured environments, we will hardcode it
# to ensure the script can run, since this seems to be the current point of failure.
PROJECT_ID = env.get("GOOGLE_CLOUD_PROJECT") or env.get("PROJECT_ID") or "vantage-468221"
...
def is_new_hash(hash_str: str) -> bool:
    ...
    try:
        ...
        # Catch any other unexpected exceptions (like PermissionDenied) to get a clear log message.
        logging.critical(f"FATAL: An unexpected error occurred while writing to Firestore: {type(e).__name__}: {e}")
        raise
...
def parse_input_payload(raw_input: str) -> dict | None:
    """
    Parses the input string, attempting Base64 decoding first,
    then falling back to plain JSON. Returns None on failure.
    """
    try:
...
def main():
    ...
    logging.info("--- Job Configuration (v4 - PROOF OF DEPLOYMENT) ---")
    ...
    permit = parse_input_payload(sys.argv[1])
    if permit is None:
        sys.exit(1)
